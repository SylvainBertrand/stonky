...
function makeOHLCV(barCount = 150, withEmas = false): OHLCVResponse {
  ...
  const emaPoints = withEmas ? bars.map((b) => ({ time: b.time, value: b.close })) : [];
  return {
    symbol: 'AAPL',
    bars,
    has_more: false,
...
describe('CandlestickChart', () => {
  ...
  it('never calls setMarkers — canvas overlay provides the label, no arrow marker needed', () => {
    render(
      <CandlestickChart data={makeOHLCV()} detections={[BULLISH_DETECTION, BEARISH_DETECTION]} />
    );
  ...
  it('renders a canvas overlay element inside the chart container', () => {
    const { container } = render(
      <CandlestickChart data={makeOHLCV()} detections={[BULLISH_DETECTION]} />
    );
