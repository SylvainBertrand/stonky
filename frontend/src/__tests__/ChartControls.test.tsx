...
describe('ChartControls', () => {
  ...
  it('each button aria-pressed reflects its individual state', () => {
    ...
    expect(screen.getByRole('button', { name: /ema 21/i }).getAttribute('aria-pressed')).toBe(
      'true'
    );
    expect(screen.getByRole('button', { name: /ema 50/i }).getAttribute('aria-pressed')).toBe(
      'false'
    );
    expect(screen.getByRole('button', { name: /supertrend/i }).getAttribute('aria-pressed')).toBe(
      'true'
    );
    expect(screen.getByRole('button', { name: /volume/i }).getAttribute('aria-pressed')).toBe(
      'false'
    );
  });
