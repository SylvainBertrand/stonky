...
describe('ScannerPage', () => {
  ...
  it('shows "Not yet scanned" status when no results have been fetched', async () => {
    server.use(http.get('/api/scanner/results', () => HttpResponse.json([])));
    renderApp();
    await waitFor(() => {
  ...
  it('shows "Scanning…" on button while scan is in progress', async () => {
    ...
    server.use(http.get('/api/scanner/runs/:runId', () => HttpResponse.json(MOCK_RUN_PENDING)));
  ...
  it('shows error banner when run status is failed', async () => {
    server.use(http.get('/api/scanner/runs/:runId', () => HttpResponse.json(MOCK_RUN_FAILED)));
    const user = userEvent.setup();
    renderApp();
    await user.click(screen.getByRole('button', { name: /run scan/i }));
    await waitFor(
  ...
  it('shows "Failed to load results" when GET /scanner/results errors', async () => {
    server.use(http.get('/api/scanner/results', () => HttpResponse.error()));
    renderApp();
    await waitFor(
      () => {
