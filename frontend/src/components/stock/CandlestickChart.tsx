...
export function drawDetectionOverlays(
  ...
  for (const det of detections) {
    ...
    let rectY = 0;
    let rectH = height;
    if (priceToCoordinate && det.price_top != null && det.price_bottom != null) {
...
export const CandlestickChart = forwardRef<ChartHandle, Props>(function CandlestickChart(
  ...
  useEffect(() => {
    ...
    if (forecastData) {
      ...
      const redrawForecast = () => {
        ...
        drawForecastOverlay(
          ...
          forecastData,
          data.bars[data.bars.length - 1].time,
          timeToCoord,
          priceToCoord,
          data.bars
        );
    ...
    // allBars / hasMore / isLoadingMore / onLoadMore intentionally omitted:
    // this effect builds the chart DOM from scratch and must NOT run on every lazy-load.
    // Dynamic bar updates are handled by the separate useEffect below.
