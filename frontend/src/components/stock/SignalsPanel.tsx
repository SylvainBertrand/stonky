...
const SIGNAL_INFO: Record<string, SignalMeta> = {
  ...
  bb_pct_b: {
    label: 'BB %B',
    category: 'volatility',
    interpret: (s) => (s > 0.5 ? 'Near lower band' : s < -0.5 ? 'Near upper band' : 'Mid range'),
  },
  ttm_squeeze: {
    label: 'TTM Squeeze',
    category: 'volatility',
  ...
  pivot_points: {
    label: 'Pivot Points',
    category: 'support_resistance',
    interpret: (s) => (s > 0.3 ? 'Near support' : s < -0.3 ? 'Near resistance' : 'Between levels'),
  },
  rsi_divergence: {
      ...
      <div className="space-y-1">
        {filtered.map((key) => {
          ...
          return (
            <div
              key={key}
              className="flex items-center gap-2 py-1 px-2 rounded hover:bg-gray-800/40"
            >
              <span className="text-xs text-gray-300 w-32 shrink-0">{info.label}</span>
              <span
                className={`text-xs font-mono font-semibold w-12 text-right ${scoreColor(score)}`}
              >
                {score.toFixed(3)}
              </span>
