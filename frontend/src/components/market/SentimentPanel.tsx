...
export function SentimentPanel() {
  ...
  useEffect(() => {
    ...
    const aaiiBullData = sentimentData.series.find(
      (s) => s.name.includes('AAII') && s.name.includes('Bull')
    );
    const aaiiBearData = sentimentData.series.find(
      (s) => s.name.includes('AAII') && s.name.includes('Bear')
    );
  ...
  if (!sentimentData || sentimentData.labels.length === 0) {
    return (
      <div className="text-center py-8 text-gray-400">
        No sentiment data available. Import AAII/NAAIM CSV via the API, or wait for scraper to
        succeed.
      </div>
    );
  }

  const aaiiBullSeries = sentimentData.series.find(
    (s) => s.name.includes('AAII') && s.name.includes('Bull')
  );
  const aaiiBearSeries = sentimentData.series.find(
    (s) => s.name.includes('AAII') && s.name.includes('Bear')
  );
  ...
  const naaimLatest =
    naaimFiltered.length > 0 ? naaimFiltered[naaimFiltered.length - 1] : undefined;
